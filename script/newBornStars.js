...
window.onload = function () {
    ...
    fetch("../app-data/library/picture-library.json")
       ...
       window.onload = function () {
           const picsFromNewbornStarsAlbum = document.querySelector('#picsFromNewbornStarsAlbum');
           
           fetch("../app-data/library/picture-library.json")
             .then((resp) => resp.json())
             .then((data) => {
               for (let i = 0; i < data.albums[2].pictures.length; i++) {
                 const src = data.albums[2].path + '/' + data.albums[2].pictures[i].imgHiRes;
             ...
             // Insert the image in picsFromNewbornStarsAlbum
             picsFromNewbornStarsAlbum.appendChild(image);
           }
         });
