    ...
    fetch("../app-data/library/picture-library.json")
        ...
        .then(function (data) {
            for (let i = 0; i < data.albums[0].pictures.length; i++) {
                picsFromGalaxiesAlbum.innerHTML += (`<img src="` + data.albums[0].path + `/` + data.albums[0].pictures[i].imgHiRes + `" id='imgId` + i + `' class='jsGeneratedAlbumImages'` + '>');
            }
        ...
        window.onload = function () {
            const copies = [];
            const picsFromGalaxiesAlbum = document.querySelector('#picsFromGalaxiesAlbum');
            
            fetch("../app-data/library/picture-library.json")
              .then((resp) => resp.json())
              .then((data) => {
                for (let i = 0; i < data.albums[0].pictures.length; i++) {
                  const src = data.albums[0].path + '/' + data.albums[0].pictures[i].imgHiRes;
              ...
              // Insert the image in picsFromGalaxiesAlbum
              picsFromGalaxiesAlbum.appendChild(image);
            }
          });
