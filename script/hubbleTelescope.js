    ...
    fetch("../app-data/library/picture-library.json")
        ...
        .then(function (data) {
            for (let i = 0; i < data.albums[4].pictures.length; i++) {
                ...
                window.onload = function () {
                    const picsFromHubbleTelescopeAlbum = document.querySelector('#picsFromHubbleTelescopeAlbum');
                    
                    fetch("../app-data/library/picture-library.json")
                      .then((resp) => resp.json())
                      .then((data) => {
                        for (let i = 0; i < data.albums[4].pictures.length; i++) {
                          const src = data.albums[4].path + '/' + data.albums[4].pictures[i].imgHiRes;
                      ...
                      // Insert the image in picsFromHubbleTelescopeAlbum
                      picsFromHubbleTelescopeAlbum.appendChild(image);
                    }
                  });
